...
def compute_reflective_uplift(
    ...
    for (task_id, _provider), pair in by_pair.items():
        ...
        # 简化判: m1 failure_bucket startswith "(max_steps"/"LOOP_DETECTED" 跟 reflect.should_reflect
        # 触发集对齐 (TRIGGERING_FAILURE_MARKERS = max_steps + LOOP_DETECTED).
        # V0.29.5 fix V0.28.3 bucket 命名 bug — runner.py:67 `marker.split(" ")[0].split(":")[0]` 处理
        # "(max_steps 耗尽未完成)" 返 "(max_steps" (带左括号), V0.28.3 `in` 检查永不命中 →
        # reflections_written 永 0. startswith 容错修.
        if not m1.pass_ and any(
            m1.failure_bucket.startswith(p)
            for p in ("(max_steps", "LOOP_DETECTED")
        ):
