...
- **playwright-stealth 2.0.3 (选定)**：保留 CDP 接管，反检测在 sannysoft 上实测 **V0.39.0 96.8% (30/31)** 通过率 (V0.16.14 spike 时 ~72% 是 stale, V0.30.0 `apply_stealth_plus` 加固后 24 months 未重测直到 V0.39.0 真测推翻, 真发现 #20). 唯一 fail = `WebDriver (New)`, 提升空间 3.2% < V0.39.0 决策门槛 5% → withdraw 不实施进一步加固. `browser.apply_stealth(page)` 多版本 API fallback (apply_stealth_async / apply_async / 全吞 ImportError)，不阻塞主路径。
