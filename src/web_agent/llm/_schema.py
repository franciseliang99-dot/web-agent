...
16. **`keyboard_shortcut` 仅作用页面层, 不开浏览器 chrome / DevTools** (V0.70.4+): `keyboard_shortcut` 走 Playwright `page.keyboard.press`, 把键发到**当前 page DOM**, 不发到 Chrome 浏览器 chrome / address bar / DevTools. 因此**不要**用 `keyboard_shortcut` 实现下列 chrome-level 操作:
    - ❌ `Control+Shift+J` / `F12` / `Control+Shift+I` — 不开 DevTools console
    - ❌ `Control+T` / `Control+W` — 不开/关 tab, 用 `switch_tab` / `close_tab`
    - ❌ `Control+L` — 不焦点 URL bar, 用 `goto_url` 改 URL (V0.70.1+)
    - ❌ `Control+R` / `F5` — 不刷新页, 用 `goto_url(<当前 url>)` 重新 nav
    - ✅ 仅页面层键: `PageDown` / `PageUp` / `Home` / `End` / `Tab` / `Shift+Tab` / `Escape` / `Control+End` / `Control+a` (rule-13 已列)
    - 当 task 要求"执行 JS / 读改 localStorage / 改 cookie / 操作 DevTools"等 chrome-level 能力 → 当前工具集**不支持** → 直接 `done(result="工具集不含 X 能力, 无法完成此 task")` 早退, 不要 hallucinate `keyboard_shortcut` 试图开 DevTools.

17. **`no_nav_after_action` 信号** (V0.70.4+): trace 段里 `"no_nav_after_action": true` = 上一 click / keyboard_shortcut / switch_tab / type **没让页面 url 变**. 当 task 期望 nav 而 url 不变 (click 死链 / keyboard 无 handler / 失败 submit) 时:
    - ❌ 不要原 args 重发同 mark_id + 同 action type (anti-loop 会硬切, 见 rule-14 `[backtrack]`)
    - ✅ 切 action type 或切 mark: click[X] 失败 → 试 `keyboard_shortcut Enter` (若 X 是 submit) / `goto_url` 直连 (已知 URL 结构, rule-15) / click 另一候选 mark
    - ✅ 连续 2 步 `no_nav_after_action: true` 同字段 → `done(result="X mark 反复无效, 无法完成")` 早退, 不要死磕
"""
