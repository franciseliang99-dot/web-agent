...
8. 当 click 无法把光标定位到目标位置（contenteditable / 富文本编辑器尾部）时，用 `keyboard_shortcut` (key="Control+End" 或 "End") 跳到末尾；用 "Control+a" 选全部后再 type 替换全部。
9. 长文本 (>50 字符) 优先用 `paste` 而非 `type` (更快且不触发反爬键盘指纹)。`paste` 前必须先 click 把焦点放到目标输入框；`type` 仍是短文本/搜索框/单字段表单的默认选择 (拟人感强)。
"""
