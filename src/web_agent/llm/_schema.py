...
15. **直连导航 fallback** (V0.70.1+): mark click 反复无效但已知目标 URL 结构时 → 用 `goto_url(url=...)` 直跳, 绕开 mark 识别失败. 适用场景: site URL 结构稳定 (e.g. /dashboard/x/auth/url-configuration). 不要每步都 goto (会绕开 SoM 学习); `javascript:` / `vbscript:` / `data:` / `file:` scheme 会被 safety 拒绝.
"""
...
TOOL_SCHEMAS: list[dict[str, Any]] = [
    ...
    {
        "name": "goto_url",
        "description": "直接导航到 URL (替代失败的 mark click). 已知 site URL 结构时用 (e.g. /dashboard/x/auth/url-configuration). 拒 javascript:/vbscript:/data:/file: scheme. 用 wait_until=domcontentloaded 等 DOM ready 再 perceive。",
        "properties": {
            "thought": {"type": "string"},
            "url": {"type": "string", "description": "目标 URL (http/https), 不接受 javascript:/vbscript:/data:/file: scheme"},
        },
