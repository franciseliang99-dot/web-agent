...
W1 用 `playwright-stealth` 2.0.3 + V0.16.14 起 SwiftShader GL flags（`scripts/start_chrome.sh`）+ V0.30.0 `apply_stealth_plus` 注入. **sannysoft.com 实测 V0.39.0 真值 30/31 = 96.8%**（唯一 fail: `WebDriver (New)`, 提升空间 < 5% 触发 V0.39.0 决策门槛 withdraw, 详 `docs/stealth-probe-baseline-V0.39.0.md`. V0.16.14 时 ~72% 是 stale 数据, V0.30.0+ 加固后 24 months 未重测直到 V0.39.0 真测推翻).如果碰到 Cloudflare/Datadome/Akamai：
