...
1. ~~切 patchright-python~~ — V0.16.14 spike 已关闭：connect_over_cdp 模式下 client patch 旁路（A=C 完全相同 19/32），spike 数据见 ARCHITECTURE §1.3。要走 patchright 必须放弃 connect_over_cdp 接管，丢失用户登录态——除非 W6 重大架构变更，否则永久 NO-GO。
2. 上住宅代理 + TLS 指纹（curl_cffi）— 这是真正未做的下一层防御
3. 验证码不接 2Captcha 自动绕（越线），用「暂停 → 弹窗让用户解 → 恢复循环」UX（W4-2 V0.9.0 已实现）
