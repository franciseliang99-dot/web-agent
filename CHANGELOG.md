...
### Refactor (V0.70.7 SYSTEM_PROMPT rule-3/16/17 终极加强 — V0.70.6 post-ship 1/4 cover 触发)

V0.70.6 dogfood POST-ship 真 cover **1/4 (25%)** — V0.70.4 rule-16/17 prompt-only 路线证明 marginal. 4 个深层根因 (post-ship subagent /audit verdict):

1. prompt 末尾 attention 弱 (rule-16/17 在 prompt 末尾)
2. LLM 字面理解 ban specific key (F12 没 generalize 到 rule-16 反例 list)
3. fallback option 喧宾夺主 (task 2 "console 或 URL bar", LLM 走 URL bar)
4. task identifier 在 goal 后段 (task 1 "console JS" 在 goal 后段 LLM 没 anchor)

V0.70.7 SYSTEM_PROMPT 3 处终极加强 (仍 prompt-only, 0 业务代码):

- **rule-3 加 early anchor** (`src/web_agent/llm/_schema.py:23`): 末尾加一句 "如 goal 含 chrome-level 关键字 (`console` / `JS 执行` / `localStorage` / `cookie` / `DevTools` / `Runtime.evaluate` / `chrome://`) → 第一步就 done 早退". rule-3 在 prompt 早段, LLM attention 强, anchor LLM 在 goal 解析阶段就识别 task 类型. 闭根因 #4.

- **rule-16 改类别 ban + goal 关键字 anchor 强复述** (`_schema.py:53`):
  - 老 "❌ Control+Shift+J / F12 / ..." (LLM literal 理解 ban specific key, F12 漏)
  - 新 "**凡是**用来开 DevTools / 操作 address bar / 切换/关闭 tab / 刷新页 的快捷键 — `keyboard_shortcut` **一律不开, 不要试任何一个**. 例子 (**不是穷举, 同类**都不该试)". **类别 ban**. 闭根因 #2.
  - 加 "**第一步就** done 早退, **不要等到撞墙再 done**, **不要试**任何 keyboard_shortcut / click 探索". 强复述 rule-3 anchor. 闭根因 #1 + #3.

- **rule-17 加连续 2 步硬约束 + V0.70.6 goto_url 信号** (`_schema.py:61`):
  - 老 "✅ 连续 2 步 no_nav_after_action: true 同字段 → done 早退" (soft 措辞 LLM 不 anchor)
  - 新 "**(V0.70.7 硬约束) 连续 2 步 no_nav 同 (mark_id / url / key) → 必须 done 放弃**. **不再尝试第 3 次**. anti-loop 会在第 3 次硬 abort, 你主动 done 比被 abort 更可控". 硬约束 + 恐吓 (anti-loop 第 3 次 abort). 闭根因 #1 + #3.
  - 信号 trigger list 加 `goto_url` 反映 V0.70.6 `_NAV_EXPECTING_ACTIONS` 改动.

### TDD (2 新测试 `tests/test_llm_schema.py`)

- `test_v0707_rule_16_category_ban_and_goal_keyword_anchor` — 验类别 ban 措辞 ("凡是" / "一律不开") + goal 关键字 anchor (`console` / `localStorage` / `Runtime.evaluate`) + "第一步" 早退指引
- `test_v0707_rule_17_consecutive_forced_done` — 验硬约束措辞 ("必须" + "放弃") + V0.70.6 goto_url 反映 + "第 3 次" / "硬 abort" 恐吓

红→绿: 2 新 case + 全 `test_llm_schema.py` 预期 25 PASS (V0.70.6 baseline 23 → +2).
...
V0.70.4 ship 后 subagent /audit 复 trace.db (用户给的 5-task batch + 我跑的 `55c9e65ea2ee`), 通过时间戳挖出 V0.70 注入 `no_nav_after_action` signal 时漏掉 `goto_url`. task 2 (`6b48e8895a0e`, 2026-05-14 18:11) 跑 10 次相同 `goto_url("http://localhost:3000/")`, `url_before == url_after` 每步成立, 但因 `goto_url not in _NAV_EXPECTING_ACTIONS`, **信号一次都没注入** trace JSON. SYSTEM_PROMPT rule-17 ("`no_nav_after_action: true` → 切 action / 切 mark / done 早退") 在 goto_url-LOOP 场景**是空话**.

V0.70.6 1 行 fix: `src/web_agent/trace.py:16` 加 `"goto_url"` 进 `_NAV_EXPECTING_ACTIONS`. 配套 `tests/test_trace_no_nav_signal.py` 加 `test_goto_url_url_unchanged_emits_no_nav_after_action` 验信号注入. baseline 9 → V0.70.6 10 PASS.

### 为什么 V0.70 / V0.70.1 漏

V0.70 注入 signal 时 `goto_url` 还**没**作为 first-class action (V0.70.1 才加 `GotoUrlAction`). V0.70.1 加 tool 时 CHANGELOG 提到 "跟 V0.21 SwitchTab/V0.23 Drag/Upload 同模式" — 但**忘**同步更新 `_NAV_EXPECTING_ACTIONS`. V0.70.4 dogfood task 2 重跑首次暴露 (subagent /audit 通过 trace.db 时间戳挖出 — V0.70.4 ship 时间 2026-05-14 18:34; user 5-task batch 4/5 是 ship 前 motivating dataset, 1 是 ship 后 mark_id 重分配 LOOP).

### V0.70.4 POST-ship 真验证样本修正

| task_id | 时间 | V0.70.4 关系 | 行为 |
|---------|------|---|------|
| `55c9e65ea2ee` (forgot-password JS) | 20:06 | POST | F12 × 1 → done 早退 (rule-16 ✅) |
| `e93e073cc60c` (首页 UI menu) | 21:14 | POST | mark[2,5,2,4,2] alternation LOOP (perceiver-level, rule-16/17 不解) |
| `7a6613ad05f3` `6b48e8895a0e` `4cfa9b20774b` `b25c34175be0` | 18:07-18:14 | PRE | 不是 V0.70.4 验证, 是 motivating |

V0.70.4 真实 POST-ship cover = 1/2 (50%), 不是 user 表的 1/5. 待 V0.70.6 ship 后**重跑 task 1-4 POST-V0.70.4+V0.70.6 真量化**.

### 不动

- SYSTEM_PROMPT rule-15/16/17 不动 (rule-17 V0.70.6 后终于在 goto_url-LOOP 触发).
- 其他 first-class action (drag/upload/paste) 不算"期望 nav", 仍不在 set, 一致.

### 排队 (V0.70.4 dogfood 暴露其他根因)

- **V0.70.7 — sign-in/auth password block 早退 prompt**: task 1 (`7a6613ad05f3` SAFETY_BLOCK on password input `safety.py:98 _DANGER_INPUT_TYPES={"password","tel"}`) — SYSTEM_PROMPT 加 rule-18 "goal 含 sign-in/auth + 拒 password type 时 done(早退)" 或 task-level whitelist env. prompt-only scope.
